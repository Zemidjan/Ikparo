...
import bj.zemidjan.dio.adapter.SuivreCoursAdapter;
...
public class SuivreCoursFragment extends Fragment  {
    public View rootView;
    RecyclerView mRecyclerView;
    public SwipeRefreshLayout mSwipeRefreshLayout;
    private ArrayList<SuivreCours> suivis;
    public ArrayList<SuivreCours> listSuivis= null;

    @Nullable
    @Override
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {

        rootView=inflater.inflate(R.layout.fragment_suivrecours,container,false);
    ...
    private void loadList(final View rootView) {
        ...
        if (search.equals("")) {
            ...
            final String userId = "" + preferences.getLong("id", 0);

            String jsonSuivi = preferences.getString("listSuivis", null);
            if (jsonSuivi != null) {
                listSuivis = gson.fromJson(jsonSuivi, new TypeToken<ArrayList<SuivreCours>>() {
                }.getType());
                suivis = listSuivis;
                mRecyclerView.setHasFixedSize(true);
                LinearLayoutManager layoutManager = new LinearLayoutManager(rootView.getContext());
                mRecyclerView.setLayoutManager(layoutManager);
                SuivreCoursAdapter adapter = new SuivreCoursAdapter(suivis, getContext());
                mRecyclerView.setAdapter(adapter);
            }
            block_loading.setVisibility(View.GONE);
            mSwipeRefreshLayout.setRefreshing(true);

            Call<ArrayList<SuivreCours>> listSuivreCours = Utils.retrofitBuilder().listSuivreCours(userId);
            listSuivreCours.enqueue(new Callback<ArrayList<SuivreCours>>() {
                @Override
                public void onResponse(Call<ArrayList<SuivreCours>> call, Response<ArrayList<SuivreCours>> response) {
                    if (response.code() == 206) {
                        final ArrayList<SuivreCours> data = response.body();
                        suivis = data;
                        if (data.isEmpty()) {
                            ...
                            message_loading.setText("Aucun cours commencé  pour le moment!");
                            mSwipeRefreshLayout.setRefreshing(false);
                        } else {
                            String jsonSuivis = gson.toJson(data);
                            editor.putString("listSuivis", jsonSuivis)
                                    .apply();
                            editor.commit();
                            mRecyclerView.setHasFixedSize(true);
                            LinearLayoutManager layoutManager = new LinearLayoutManager(rootView.getContext());
                ...
                @Override
                public void onFailure(Call<ArrayList<SuivreCours>> call, Throwable t) {
                    block_loading.setVisibility(View.GONE);
                    message_loading.setVisibility(View.GONE);
            ...
            mSwipeRefreshLayout.setRefreshing(true);
            block_loading.setVisibility(View.GONE);
            String jsonSuvis = preferences.getString("listSuivis", null);
            if (jsonSuvis != null) {
                listSuivis = gson.fromJson(jsonSuvis, new TypeToken<ArrayList<SuivreCours>>() {
                }.getType());

                //jsonDomaines = gson.fromJson(jsonDomaines, new TypeToken<ArrayList<Domaine>>() {
                //}.getType());
                final ArrayList<SuivreCours> resultSuivi = new ArrayList<SuivreCours>();
                final ArrayList<SuivreCours> resultSearchSuivi = listSuivis;
                int resultSearchSuiviLength = resultSearchSuivi.size();

                for (int i = 0; i < resultSearchSuiviLength; i++) {
                    if ((resultSearchSuivi.get(i).getCours().getTitre().toLowerCase().contains(search.toLowerCase())) ||
                            (resultSearchSuivi.get(i).getCours().getDescription().toLowerCase().contains(search.toLowerCase())) ||
                            (resultSearchSuivi.get(i).getCours().getDomaine().getLibelle().toLowerCase().contains(search.toLowerCase()))
                        ) {
                        resultSuivi.add(resultSearchSuivi.get(i));
                ...
                mRecyclerView.setLayoutManager(layoutManager);
                SuivreCoursAdapter adapter = new SuivreCoursAdapter(resultSuivi, getContext());
                mRecyclerView.setAdapter(adapter);
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        ((AppCompatActivity)getActivity()).getSupportActionBar().setTitle("Mes cours Suivis");
